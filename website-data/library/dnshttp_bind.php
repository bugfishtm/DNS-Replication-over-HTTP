<?php
	...
	function dnshttp_bind_domain_id_exists($mysql, $id) { 
		if(is_numeric($id)) { 
	...
	function dnshttp_bind_domain_name_exists($mysql, $domain_name) { 
		if(trim($domain_name) != "") { 
	...
	function dnshttp_bind_get($mysql, $id) {
		if(is_numeric($id)) { 
			$query = mysqli_query($mysql->mysqlcon, "SELECT * FROM "._TABLE_DOMAIN_BIND_." WHERE id = \"".$mysql->escape($id)."\"");
	...
	function dnshttp_bind_delete_by_id($mysql, $id) {
	...
	// Register a New Domain
	function dnshttp_bind_delete_by_name($mysql, $domain) {
		// If Exists, Do not Register
